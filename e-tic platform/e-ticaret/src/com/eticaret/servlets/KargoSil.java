...
public class KargoSil extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.setCharacterEncoding("UTF-8");
			...
			kargo.setKargoId(Integer.parseInt(request.getParameter("kargoId"))); //formdan namesi kargoId olan veriyi çek
			try {
				int state = KargoDao.kargoSil(kargo);
				String message = (state == 1) ? "Kargo şirketi başarıyla silindi" : "İşlem sırasında bir hata oluştu";
				request.setAttribute("stateSilme", message); //mesaj stringini başka sayfada kullanmak için attr oluşturuyor
