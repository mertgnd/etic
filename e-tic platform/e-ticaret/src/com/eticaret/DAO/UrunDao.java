...
public class UrunDao {
	...
	public static ArrayList<Urun> cokSatilanGetir() throws SQLException {
		ArrayList<Urun> coksatilanlar = new ArrayList<Urun>();
		try {
			con = ConnectionDao.getConnection();
			psmt = con.prepareStatement("Select * from Urun where Kategori_kategoriId=1;");
			rs = psmt.executeQuery();
			while (rs.next()) {
				Urun urun = new Urun();
				urun.setUrunId(rs.getInt(1));
				...
				urun.setUrunFiyat(rs.getInt(4));
				urun.setUrunMiktar(rs.getInt(5));
				urun.setKategori_kategoriId(rs.getInt(6));
				urun.setMarka_markaId(rs.getInt(7));
				urun.setUrunResim(rs.getBinaryStream(8));
